  ...
  if (response.response_type() == MPIResponse::ALLGATHER) {
    ...
    if (on_gpu) {
      ...
      if (horovod_global.hierarchical_allreduce) {
        ...
        int64_t num_elements_per_gpu = (num_elements%horovod_global.local_size == 0 ? (num_elements/horovod_global.local_size) : (num_elements/horovod_global.local_size + 1) );
        ...
        if (horovod_global.local_rank >= 0) {
          ...
          // a buffer is not safe since the tensor can be arbitrarily large.
           ...
           buffer_len_per_proc = (buffer_len/num_elements)*num_elements_per_gpu;
           host_buffer = malloc(buffer_len_per_proc);

          if(timeline.Initialized()) {ACTIVITY_START_ALL(entries, timeline, CUDA_MEMCPY_ASYNC)}
          CUDA_CHECK(entries, "cudaMemcpyAsync",
                     cudaMemcpyAsync(host_buffer, buffer_data, buffer_len_per_proc,
                                     cudaMemcpyDeviceToHost, stream))

          if(timeline.Initialized()) {ACTIVITY_END_ALL(entries, timeline)}
